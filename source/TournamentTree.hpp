...
vector<node> Construct(array* leaves, int& counter);            // funkcja budująca strukturę drzewa turniejowego
...
void Replacemax(vector<node>& tree, int newVal, int& counter);  // funkcja wstawiająca nową wartość na miejsce starego maximum drzewa turniejowego
void HadianSobel(array* list, int k, int& counter);             // funkcja wykonująca algorytm hadiana-Sobela wyznaczając k największych wartości
...
vector<node> Construct(array* leaves, int& counter) {
...
void Replacemax(vector<node>& tree, int newVal, int& counter) {
...
void HadianSobel(array* list, int k, int &counter) {
  // rozmiar listy wejściowej
  int n = list->size; 
  // wydzielenie tablicy o rozmiarze n - k + 1
  array leaves = {list->l, short(n - k + 1)};
  // tworzenie drzewa
  vector<node> tree = Construct(&leaves, counter);
  // czytanie oraz wywołanie Replacemax k - 1 razy 
  for (int i = n - k + 1; i < n; i ++) {
 //   cout << val(1) << SEPARATOR;
    Replacemax(tree, leaves.l[i], counter);
  }
}
